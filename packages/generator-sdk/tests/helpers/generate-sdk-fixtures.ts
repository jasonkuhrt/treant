...
const generateSdkFixtures = async () => {
  ...
  for (const caseDir of caseDirs) {
    ...
    const artifacts = await Grammar.generateAsync(grammar, { wasm: true, cache: true });

    const sdk = await generate({
      grammar: artifacts['grammar.json'].parsed,
      nodeTypes: artifacts['node-types.json'].parsed,
      wasm: artifacts['parser.wasm'],
    });
