...
import { analyzeGrammar } from './generators/core/grammar-analysis.js';
import { DEFAULT_CURSOR_CONFIG, generateCursorSystem } from './generators/cursor/cursor-generator.js';
 ...
 */
const ensureDir = (path: string) => Effect.gen(function* () {
  const fs = yield* FileSystem.FileSystem;
 ...
 */
const loadJson = <T = unknown>(path: string) => Effect.gen(function* () {
  const fs = yield* FileSystem.FileSystem;
 ...
 */
export const generate = (options: GenerateOptions) => Effect.gen(function* () {
  const { grammarPath, nodeTypesPath, outputDir, nameOverride } = options;
  const fs = yield* FileSystem.FileSystem;
  const path = yield* Path.Path;

  // Load grammar and node types
  const grammarJson = yield* loadJson<Grammar.Types.GrammarJson>(grammarPath);
  const nodeTypes = yield* loadJson<NodeType[]>(nodeTypesPath);
  ...
  if (existsSync(cacheFile)) {
    ...
    formatter = yield* Effect.promise(() => 
      createStreaming(Promise.resolve(new Response(wasmBuffer)))
    );
  } else {
    ...
    formatter = yield* Effect.promise(() => 
      createStreaming(Promise.resolve(new Response(buffer)))
    );
  }
  ...
  const grammarAnalysis = analyzeGrammar(nodeTypes, grammarJson as any);
  ...
  yield* Effect.promise(() =>
    generateCursorSystem(cursorConfig, grammarAnalysis, DEFAULT_CURSOR_CONFIG)
  );
}).pipe(
...
function generateNodeFile(node: NodeType, grammarName: string, grammarNamePascal: string): string {
...
function generateUtilsFile(nodeTypes: NodeType[], grammarName: string): string {
...
function generateAnonymousNodesFile(anonymousNodes: NodeType[], grammarName: string): string {
  // Categorize anonymous nodes
  const punctuationNodes = anonymousNodes.filter(node =>
    node.type.length === 1 && /^[^a-zA-Z0-9]$/.test(node.type)
  );
  
  const operatorNodes = anonymousNodes.filter(node =>
    node.type.length > 1 && /^[^a-zA-Z0-9]+$/.test(node.type)
  );
  
  const keywordNodes = anonymousNodes.filter(node => 
    /^[a-z]+$/.test(node.type)
  );
