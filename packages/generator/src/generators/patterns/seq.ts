 ...
 */
export function isSeqPattern(rule: Grammar.Rule): rule is Grammar.Rules.Seq {
...
export function isOptionalPattern(rule: Grammar.Rule): boolean {
  ...
  const choiceRule = rule as Grammar.Rules.Choice;
 ...
 */
export function extractSeqMembers(rule: Grammar.Rules.Seq): SeqMember[] {
  ...
  for (const member of rule.members) {
    ...
    if (isOptionalPattern(member)) {
      ...
      if (innerRule && innerRule.type === 'SYMBOL') {
        members.push({
          name: (innerRule as Grammar.Rules.Symbol<string>).name,
          optional: true,
          rule: innerRule
        });
      } else if (innerRule && innerRule.type === 'FIELD') {
        // Handle FIELD wrapped in optional
        const fieldRule = innerRule as Grammar.Rules.Field;
        members.push({
          name: fieldRule.name,
          optional: true,
          rule: innerRule
    ...
    // Handle direct SYMBOL references
      ...
      members.push({
        name: (member as Grammar.Rules.Symbol<string>).name,
        optional: false,
        rule: member
      });
    }
    // Handle FIELD definitions
      ...
      const fieldRule = member as Grammar.Rules.Field;
      members.push({
        name: fieldRule.name,
        optional: false,
        rule: member
...
export function analyzeNodeForSeqPattern(
  ...
  grammarJson: Grammar.GrammarJson
): { hasSeq: boolean; rule?: Grammar.Rules.Seq; members?: SeqMember[] } {
  ...
  if (rule.type === 'SYMBOL') {
    const symbolRule = rule as Grammar.Rules.Symbol<string>;
