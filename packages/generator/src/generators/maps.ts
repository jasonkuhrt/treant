...
import { emitInterfaceProperty, emitTupleType } from '../lib/ts-syntax/ts-syntax.js';
import { emitUnionType } from '../lib/ts-syntax/ts-syntax.js';
    ...
    if (typeof rule === 'object' && rule !== null && rule.type === 'SEQ' && rule.members) {
      ...
      rule.members.forEach(member => {
        if (member.type === 'SYMBOL') {
          ...
          const hasBlank = member.members.some(m => m.type === 'BLANK');
          const symbolMember = member.members.find(m => m.type === 'SYMBOL');

          if (hasBlank && symbolMember && symbolMember.type === 'SYMBOL') {
