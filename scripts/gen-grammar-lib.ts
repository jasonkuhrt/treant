...
import grammarJson from '../parser/src/grammar.json' with { type: 'json' };
...
const grammarNamePascal = overrideName || (grammarName === 'graphql' ? 'GraphQL' : toPascalCase(grammarName));
 ...
 */
function getNodeInterfaceName(nodeType: string): string {
  const pascalType = toPascalCase(nodeType);
  const interfaceName = pascalType; // Removed 'Node' suffix
 ...
 */
function getTypeGuardName(nodeType: string): string {
 ...
 */
function needsAliasing(nodeType: string): boolean {
  const interfaceName = toPascalCase(nodeType); // Removed 'Node' suffix
 ...
 */
function generateNamespaceAlias(nodeType: string): string | null {
...
function generateInterfaceJSDoc(node: NodeType): string {
  ...
  // Grammar-specific examples could be added here based on grammar name
  // For now, we'll keep the interface documentation generic
...
function generateTypeGuardJSDoc(node: NodeType): string {
  ...
  return [
    ...
    ' * @example',
    ' * ```typescript',
    ` * if (${getTypeGuardName(node.type)}(node)) {`,
...
function generateAnonymousNodesFile(
  ...
  if (punctuationTypes.length > 0) {
    lines.push('/**');
    lines.push(` * Single-character punctuation tokens in ${grammarName}.`);
    lines.push(' */');
    lines.push(`export type PunctuationType = ${punctuationTypes.map(t => `'${t}'`).join(' | ')};`);
    lines.push('');

    lines.push('/**');
    lines.push(` * Interface for punctuation nodes in the ${grammarName} AST.`);
    lines.push(' */');
    ...
    lines.push('/**');
    lines.push(' * Type guard to check if a node is a {@link Punctuation}.');
    lines.push(' */');
    ...
    lines.push(' */');
    lines.push(`export const PUNCTUATION_TYPES = [${punctuationTypes.map(t => `'${t}'`).join(', ')}] as const;`);
    lines.push('');
  }
  ...
  if (operatorTypes.length > 0) {
    ...
    lines.push(' */');
    lines.push(`export const OPERATOR_TYPES = [${operatorTypes.map(t => `'${t}'`).join(', ')}] as const;`);
    lines.push('');
  }
  ...
  if (keywordTypes.length > 0) {
    ...
    lines.push(' */');
    lines.push(`export const KEYWORD_TYPES = [${keywordTypes.map(t => `'${t}'`).join(', ')}] as const;`);
    lines.push('');
  }
  ...
  if (otherTypes.length > 0) {
    ...
    lines.push(' */');
    lines.push(`export const OTHER_ANONYMOUS_TYPES = [${otherTypes.map(t => `'${t}'`).join(', ')}] as const;`);
    lines.push('');
  }

  // Generate union type for all anonymous nodes
  lines.push('/**');
  lines.push(` * Union type of all anonymous nodes in the ${grammarName} AST.`);
  lines.push(' */');
...
function generateUtilsFile(): string {
  ...
  lines.push('/**');
  lines.push(' * Find the first child node of a specific type.');
  lines.push(' * ');
  lines.push(' * @param node - The parent node to search in');
  lines.push(' * @param type - The type of node to find');
  lines.push(' * @returns The first matching child node with proper type, or null if not found');
  lines.push(' * ');
  lines.push(' * @example');
  lines.push(' * ```typescript');
  lines.push(' * const field = findChildByType(selectionSet, \'field\');');
  lines.push(' * // field is typed as FieldNode | null');
  lines.push(' * if (field) {');
  lines.push(' *   console.log(field.type); // "field"');
  lines.push(' * }');
  lines.push(' * ```');
  lines.push(' */');
  ...
  lines.push('/**');
  lines.push(' * Find all child nodes of a specific type.');
  lines.push(' * ');
  lines.push(' * @param node - The parent node to search in');
  lines.push(' * @param type - The type of nodes to find');
  lines.push(' * @returns Array of matching child nodes with proper types');
  lines.push(' * ');
  lines.push(' * @example');
  lines.push(' * ```typescript');
  lines.push(' * const fields = findChildrenByType(selectionSet, \'field\');');
  lines.push(' * // fields is typed as FieldNode[]');
  lines.push(' * fields.forEach(field => {');
  lines.push(' *   console.log(field.type); // "field"');
  lines.push(' * });');
  lines.push(' * ```');
  lines.push(' */');
  ...
  lines.push('/**');
  lines.push(' * Find a child node by following a path of node types.');
  lines.push(' * ');
  lines.push(' * @param node - The root node to start from');
  lines.push(' * @param path - Array of node types to follow');
  lines.push(' * @returns The node at the end of the path, or null if path doesn\'t exist');
  lines.push(' * ');
  lines.push(' * @example');
  lines.push(' * ```typescript');
  lines.push(' * // Navigate: fragment_definition -> type_condition -> named_type -> name');
  lines.push(' * const name = findChildByPath(fragmentDef, [\'type_condition\', \'named_type\', \'name\']);');
  lines.push(' * ```');
  lines.push(' */');
  ...
  lines.push('/**');
  lines.push(' * Generic tree walker that visits every node in the AST.');
  lines.push(' * ');
  lines.push(' * @param node - The root node to start walking from');
  lines.push(' * @param visitor - Function called for each node');
  lines.push(' * ');
  lines.push(' * @example');
  lines.push(' * ```typescript');
  lines.push(' * walkTree(document, (node) => {');
  lines.push(' *   if (node.type === \'field\') {');
  lines.push(' *     console.log(\'Found field:\', node.text);');
  lines.push(' *   }');
  lines.push(' * });');
  lines.push(' * ```');
  lines.push(' */');
  ...
  lines.push('/**');
  lines.push(' * Create a configured GraphQL parser instance for Node.js or browser environments.');
  lines.push(' * ');
  lines.push(' * This helper handles the complex WASM loading logic and provides a ready-to-use');
  lines.push(' * parser with the GraphQL grammar loaded.');
  lines.push(' * ');
  lines.push(' * @param options - Optional configuration for WASM file paths');
  lines.push(' */');
  ...
  lines.push(
    '    const graphqlWasmPath = options?.graphqlWasmPath ?? path.join(process.cwd(), \'node_modules/tree-sitter-graphql-grammar-wasm/grammar.wasm\')',
  );
  lines.push('');
...
function generateAccessorsFile(): string {
  ...
  for (const [ruleName, ruleDefinition] of Object.entries(grammarJson.rules)) {
    if (typeof ruleDefinition === 'object' && ruleDefinition !== null) {
      const rule = ruleDefinition as any;
      if (rule.type === 'SEQ' && rule.members) {
        ...
        if (ruleFunctions.length > 0) {
          ...
          usedTypes.add(getNodeInterfaceName(ruleName));
...
function detectSemanticGroupings(grammarRules: any): Map<string, string[]> {
  const groupings = new Map<string, string[]>();

  for (const [ruleName, rule] of Object.entries(grammarRules)) {
    if (typeof rule === 'object' && rule !== null && rule.type === 'CHOICE' && rule.members) {
      // Check if all members are SYMBOL references
      const symbolMembers = rule.members.filter((member: any) => member.type === 'SYMBOL');
      if (symbolMembers.length === rule.members.length && symbolMembers.length > 1) {
        // This is a semantic grouping
        const memberNames = symbolMembers.map((member: any) => member.name);
        groupings.set(ruleName, memberNames);
      }
    }
  }
...
function generateAccessorFunctionsForRule(
  ...
  const nodeTypeName = getNodeInterfaceName(ruleName);

  // Collect all possible child types from the rule
  const childTypes: Array<{ type: string; isOptional: boolean }> = [];

  rule.members.forEach((member: any) => {
    if (member.type === 'SYMBOL') {
      ...
      if (hasBlank && member.members?.length === 2) {
        const symbolMember = member.members.find((m: any) => m.type === 'SYMBOL');
        if (symbolMember) {
          childTypes.push({ type: symbolMember.name, isOptional: true });
        }
      }
    }
  });
  ...
  childTypes.forEach(({ type: childType }) => {
    const accessorName = `get${toPascalCase(childType)}From${toPascalCase(ruleName)}`;
    const childNodeType = getNodeInterfaceName(childType);
...
function generateMetadataFile(): string {
  ...
  lines.push('import type * as WebTreeSitter from \'web-tree-sitter\';');
  lines.push('import grammarJson from \'./parser/src/grammar.json\' with { type: \'json\' };');
  lines.push('');
...
function generateLayer3CompositeExtractors(lines: string[]): void {
  lines.push('/**');
  lines.push(' * Extract all nested fields from a selection set.');
  lines.push(' * ');
  lines.push(' * @param node - Selection set node to extract from');
  lines.push(' * @returns Array of field nodes found in the selection set');
  lines.push(' */');
  ...
  lines.push('/**');
  lines.push(' * Extract all directive nodes from a subtree.');
  lines.push(' * ');
  lines.push(' * @param node - Root node to search from');
  lines.push(' * @returns Array of directive nodes');
  lines.push(' */');
  ...
  lines.push('/**');
  lines.push(' * Extract all variable references from a subtree.');
  lines.push(' * ');
  lines.push(' * @param node - Root node to search from');
  lines.push(' * @returns Array of variable nodes');
  lines.push(' */');
...
function generateLayer4StructureMetadata(lines: string[]): void {
  lines.push('/**');
  lines.push(' * Get structural information about a node.');
  lines.push(' * ');
  lines.push(' * @param node - The node to analyze');
  lines.push(' * @returns Metadata about the node structure');
  lines.push(' */');
  ...
  lines.push('/**');
  lines.push(' * Calculate the depth of a node in the tree.');
  lines.push(' * ');
  lines.push(' * @param node - The node to measure');
  lines.push(' * @returns Depth from root (0-based)');
  lines.push(' */');
  ...
  lines.push('  const rule = (grammarJson.rules as any)[nodeType];');
  lines.push('  if (!rule) return [];');
  lines.push('  ');
  lines.push('  const childTypes: string[] = [];');
  lines.push('  ');
  lines.push('  function extractTypesFromRule(r: any): void {');
  lines.push('    if (r.type === \'SYMBOL\') {');
  lines.push('      childTypes.push(r.name);');
  lines.push('    } else if (r.type === \'SEQ\' && r.members) {');
  lines.push('      r.members.forEach(extractTypesFromRule);');
  lines.push('    } else if (r.type === \'CHOICE\' && r.members) {');
  lines.push('      r.members.forEach(extractTypesFromRule);');
  lines.push('    } else if (r.type === \'REPEAT\' || r.type === \'REPEAT1\') {');
  lines.push('      extractTypesFromRule(r.content);');
  lines.push('    }');
  lines.push('  }');
  lines.push('  ');
  lines.push('  extractTypesFromRule(rule);');
  lines.push('  return [...new Set(childTypes)]; // Remove duplicates');
  lines.push('}');
  lines.push('');
  ...
  lines.push('      // Check if it\'s an anonymous node (not in grammar rules)');
  lines.push('      if (!(grammarJson.rules as any)[child.type]) {');
  ...
  lines.push('/**');
  lines.push(' * Get a summary of the AST structure for debugging.');
  lines.push(' * ');
  lines.push(' * @param node - Root node to summarize');
  lines.push(' * @param maxDepth - Maximum depth to traverse (default: 3)');
  lines.push(' * @returns String representation of the tree structure');
  lines.push(' */');
...
(async () => {
  try {
    ...
    mkdirSync(join(projectRoot, 'grammar-lib/cursor'), { recursive: true });
...
`;
    ...
    await writeFormattedFile(join(projectRoot, 'grammar-lib/anonymous-nodes.ts'), anonymousNodesContent, formatter);
    console.log('Generated grammar-lib/anonymous-nodes.ts');

    // Generate Layer 1: Utils file
    const utilsContent = generateUtilsFile();
    await writeFormattedFile(join(projectRoot, 'grammar-lib/utils.ts'), utilsContent, formatter);
    console.log('Generated grammar-lib/utils.ts');

    // Generate Layer 2: Accessors file
    const accessorsContent = generateAccessorsFile();
    await writeFormattedFile(join(projectRoot, 'grammar-lib/accessors.ts'), accessorsContent, formatter);
    console.log('Generated grammar-lib/accessors.ts');

    // Generate Layer 3: Extractors file
    const extractorsContent = generateExtractorsFile();
    await writeFormattedFile(join(projectRoot, 'grammar-lib/extractors.ts'), extractorsContent, formatter);
    console.log('Generated grammar-lib/extractors.ts');

    // Generate Layer 4: Metadata file
    const metadataContent = generateMetadataFile();
    await writeFormattedFile(join(projectRoot, 'grammar-lib/metadata.ts'), metadataContent, formatter);
    console.log('Generated grammar-lib/metadata.ts');
    ...
    const grammarAnalysis = analyzeGrammar(nodeTypes, grammarJson);
    ...
    const cursorConfig = {
      ...
      formatter,
      outputDir: join(projectRoot, 'grammar-lib'),
    };
    ...
    const semanticGroupings = detectSemanticGroupings(grammarJson.rules);

    // Create node directory
    const nodeDir = join(projectRoot, 'grammar-lib/node');
    ...
    ruleNames.forEach(type => {
      nodeBarrelLines.push(`export type { ${getNodeInterfaceName(type)} } from '../nodes/${type}.js';`);
      nodeBarrelLines.push(`export { ${getTypeGuardName(type)} } from '../nodes/${type}.js';`);
    });
    ...
    console.log('Generated grammar-lib/node/$$.ts');
...
`;
    ...
    if (semanticGroupings.size > 0) {
      ...
      Array.from(allMemberNames).sort().forEach(name => {
        const nodeName = getNodeInterfaceName(name);
      ...
      Array.from(allMemberNames).sort().forEach(name => {
        const guardName = getTypeGuardName(name);
      ...
      for (const [groupName, memberNames] of semanticGroupings) {
        const groupNamePascal = toPascalCase(groupName);
        const memberTypes = memberNames.map(name => '_' + getNodeInterfaceName(name)).join(' | ');
      ...
      for (const [groupName, memberNames] of semanticGroupings) {
        ...
        const memberChecks = memberNames.map(name => `_${getTypeGuardName(name)}(node)`).join(' || ');

        // Improved JSDoc that lists all member types
        const memberTypesList = memberNames.map(name => getNodeInterfaceName(name)).join(', ');
      ...
      ruleNames.forEach((type, index) => {
        const nodeName = getNodeInterfaceName(type);
        const separator = index === 0 ? ' ' : '| ';
      ...
      const namedChecks = ruleNames.map(type => `_${getTypeGuardName(type)}(node)`);
      ...
      console.log('Generated grammar-lib/node/group/$$.ts');
...
`;
    ...
    const aliases = ruleNames
      .map(type => generateNamespaceAlias(type))
    ...
    // Export anonymous nodes and 4-layer architecture
    mainBarrelLines.push('');
    mainBarrelLines.push('// Export anonymous nodes and 4-layer architecture');
    mainBarrelLines.push(`export * from './anonymous-nodes.js';`);
    mainBarrelLines.push('');
    ...
    mainBarrelLines.push('');
    mainBarrelLines.push('/**');
    mainBarrelLines.push(` * Namespace containing all ${grammarName} AST node types and type guards.`);
    mainBarrelLines.push(' * This provides a cleaner API by grouping all node-related exports.');
    mainBarrelLines.push(' */');
    mainBarrelLines.push('export * as Node from \'./node/$.js\';');

    await writeFormattedFile(join(projectRoot, 'grammar-lib/$$.ts'), mainBarrelLines.join('\n'), formatter);
 ...
 * @example
 * \`\`\`typescript
 * import { ${grammarNamespaceExport} } from './grammar-lib/nodes/$.js';
 * 
