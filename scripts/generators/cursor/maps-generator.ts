...
export async function generateCursorMaps(
  ...
  // Generate runtime maps for actual cursor implementation
  lines.push('/**');
  lines.push(' * Runtime child relationship map for cursor navigation.');
  lines.push(' */');
  lines.push('export const CHILD_MAP: Record<string, readonly string[]> = {');

  analysis.childRelationships.forEach((children, parent) => {
    const childArray = Array.from(children).sort();
    if (childArray.length > 0) {
      lines.push(`  '${parent}': [${childArray.map(c => `'${c}'`).join(', ')}],`);
    }
  });
  ...
  lines.push('/**');
  lines.push(' * Runtime parent relationship map for cursor navigation.');
  lines.push(' */');
  lines.push('export const PARENT_MAP: Record<string, readonly string[]> = {');

  analysis.parentRelationships.forEach((parents, child) => {
    const parentArray = Array.from(parents).sort();
    if (parentArray.length > 0) {
      lines.push(`  '${child}': [${parentArray.map(p => `'${p}'`).join(', ')}],`);
    }
  });
